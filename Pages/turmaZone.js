...
export function turmaZone() {
  ...
  document.getElementById("title-turma-nome").innerHTML =
    turma.turmaNome === "guia"
      ? "<h1>Guia de Turismo</h1>"
      : `<h1>${
          turma.turmaNome.charAt(0).toUpperCase() + turma.turmaNome.slice(1)
        }</h1>`;
...
function handleDeletarAluno(aluno) {
  if (turma) {
    ...
    if (turma.deleteAluno(aluno.nome)) {
      ...
      alert(
        `Algo deu errado ao deletar ${aluno.nome} da turma de ${turma.turmaNome}`
      );
    }
  }
}
  ...
  if (turma) {
    ...
    alunos.forEach((aluno) => {
      ...
      for (let [key, value] of Object.entries(aluno)) {
        ...
        row.appendChild(
          criarCelula(key === "aprovado" ? (value ? "sim" : "não") : value)
        );
      }
